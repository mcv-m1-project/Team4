...
for i = 1:length(method)
    ...
    Specifity = mean(method{i,1}.Specifity);
    Sensitivity = mean(method{i,1}.Sensivity);
    F1 = mean(method{i,1}.F1);
    Recall = mean(method{i,1}.Recall);
    
    Metodo = struct('NumberTP', TP, 'NumberFP', FP, 'NumberFN', FN, 'NumberTN', TN, 'Precision', Precision, 'Accuracy', Accuracy, 'Specifity', Specifity, 'Sensivity', Sensitivity,'F1', F1,'Recall', Recall);
